...
class KeyListen(ConcurrentQueue<ushort> KeyQueue , CustomKeyboardEffect effect)
{
    public void KeyListener(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            if (KeyQueue.TryDequeue(out ushort rawcode))
            {
                Key key = ConvertKey(((KeyboardKeys)rawcode));
                effect[key] = Color.White;
            }
            else
            {
    ...
    Key ConvertKey(KeyboardKeys key)
    {
        switch (key)
        {
